...
var recorder = function(source, silenceDetectionConfig) {
  ...
  // Create a ScriptProcessorNode with a bufferSize of 4096 and a single input and
  // output channel
  ...
  var exportWAV = function(callback, sampleRate) {
    ...
    worker.postMessage({ command: "export", sampleRate: sampleRate });
  };
  ...
  return { record: record, stop: stop, clear: clear, exportWAV: exportWAV };
};
...
const audioRecorder = function() {
  ...
  return { requestDevice, createRecorder, audioContext };
};
