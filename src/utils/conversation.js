...
const conversation = function(
  ...
  this.transition = function(conversation) {
    ...
    // If we are transitioning into SENDING or SPEAKING we want to immediately
    // advance the conversation state to start the service call or playback.
    if (
      state.message === state.messages.SENDING ||
      state.message === state.messages.SPEAKING
    ) {
    ...
    // If we are transitioning in to sending and we are not detecting silence (this
    // was a manual state change) we need to do some cleanup: stop recording, and
    // stop rendering.
    if (
      state.message === state.messages.SENDING &&
